...
.. tip::
    ...
    Visit `documentation for this SDK module <https://sdk.hardwario.com/group__bc__module__relay.html>`_
...
First you have to instantiate variable typed ``bc_module_relay_t`` which represents the relay and use the *relay init* function.
...
You can toggle the states manually. That is achieved by calling the ``bc_module_relay_toggle(bc_module_relay_t *self)``,
where the *self* parameter is an address to instantiated variable of type ``bc_module_relay_t``.
...
More interesting is control with the ``bc_module_relay_pulse(bc_module_relay_t *self, bool direction, bc_tick_t duration)`` function.
